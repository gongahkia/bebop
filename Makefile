...
test-integration:
	@if docker info >/dev/null 2>&1; then \
		BEBOP_INTEGRATION_DOCKER=1 go test -tags=integration -run '^Test(Debian|Devuan|Fedora|EnterpriseLinux|OpenSUSE|Arch|Void)Inspect$$|^TestAlpine(Inspect|APKRepositorySyntax)$$|^TestVoidXBPSPackageMetadata$$' ./internal/integration; \
	else \
