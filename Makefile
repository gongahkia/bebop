...
test-integration:
	@if docker info >/dev/null 2>&1; then \
		BEBOP_INTEGRATION_DOCKER=1 go test -tags=integration -run '^Test(Debian|Fedora|EnterpriseLinux|OpenSUSE|Arch)Inspect$$|^TestAlpine(Inspect|APKRepositorySyntax)$$' ./internal/integration; \
	else \
